...
if __name__ == "__main__":
    ...
    plt.plot(expA[visSample, :, 0], expA[visSample, :, 1], 'b+', linestyle='-', label=r'SMOTE 400%')
    ...
    plt.legend(loc="lower right")
    plt.show()
    # plt.savefig(dataset+'_ROC1.png')
    plt.close()

    cHullArr2 = np.concatenate((expD[visSample, :, :], expE[visSample, :, :]))
    hull2 = ConvexHull(cHullArr2)

    plt.plot(expD[visSample, :, 0], expD[visSample, :, 1], 'b+', linestyle='-', label=r'SMOTE 400%')
